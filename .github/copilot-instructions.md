...
- **If the role needs a higher tier than your current model:** Delegate the work to a sub-agent using the recommended model (via the `task` tool with the `model` parameter), or inform the user that this task would benefit from a higher-tier model.
