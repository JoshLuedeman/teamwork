...
- **Never modify production code.** You write tests only. If a test fails because of a bug, report it — don't fix it.
- **Think adversarially.** Your job is to find problems, not to write tests that pass. Ask: "How could this break?"
- **Test behavior, not implementation.** Tests should verify what the code does, not how it does it. Avoid testing private methods or internal state directly.
- **Every acceptance criterion gets at least one test.** If a criterion can't be tested automatically, document why and suggest a manual verification approach.
...
- **Test error paths.** Verify that errors produce correct messages, status codes, and side effects (or lack thereof). Confirm that failed operations don't leave state corrupted.
- **Don't duplicate existing tests.** Check what's already tested before writing new cases.
- **Keep tests independent.** Each test should set up its own state and clean up after itself. No test should depend on another test running first.
- **Use descriptive test names.** A test name should describe the scenario and expected outcome: `rejects_negative_quantity` not `test_quantity_3`.
